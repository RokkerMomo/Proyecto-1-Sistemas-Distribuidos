...
const product = new Servicio("localhost:3003",grpc.credentials.createInsecure());
...
//Crear Producto
    product.createProduct(data_product, function (err, response) {
        if(err){
          ...
          parentPort.postMessage('se creo el producto worker3');}
        product.getProducts({}, (err, results) => {
            if (!err) {
...
product.updateProduct(data_product2, function (err,response){
    if (err) {
        ...
        product.getProducts({}, (err, results) => {
            if (!err) {
                console.log(results.Products);
                parentPort.postMessage('se edito el producto worker3');
                }
                });
    }
    ...
    product.deleteProduct(data_product3, function (err, response) {
        if(err){ 
          ...
          parentPort.postMessage('se Borro el producto worker3');}
    })
    parentPort.postMessage('se Borro el producto worker3');
        break;
  }
